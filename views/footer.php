		</div>
		<footer class="page-footer primary-dark">
			<div class="container">
				<div class="row">
					<div class="col l12 m12 s12">
						<h5 class="white-text"><img id="footerImg" src="./images/favicon-big.png" style="vertical-align: middle;"><span id="footerBtg"> BTG</span></h5>
					</div>
					<p class="grey-text text-lighten-4"><?php echo _WHY;?><br>
						<?php echo _FULL_CODE_1;?><a href="https://github.com/Alexandre-Moreau/projet-s6" target="_blank"><?php echo _FULL_CODE_2;?></a></p>
					<p>
						<?php echo _ICON;?> <a href="https://www.flaticon.com/authors/freepik" target="_blank">Freepik</a> <?php echo _FROM;?> www.flaticon.com<br/>
						<?php echo _VECTOR;?> <a href="https://www.flaticon.com/authors/smashicons" target="_blank">Smashicons</a>, <a href="https://www.flaticon.com/authors/icon-works" target="_blank">Icon Works</a> <?php echo _FROM;?> www.flaticon.com
					</p>
				</div>
			</div>

			<div class="footer-copyright">
				<div class="container">
					© 2018 Copyright BTG
				</div>
			</div>
		</footer>
	</body>
</html>
