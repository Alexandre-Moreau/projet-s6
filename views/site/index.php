<!-- <div class="jumbotron">
	<h1 class="display-3"><?php echo _ACCUEIL;?></h1>
	<p class="lead"><?php echo _BIENVENUE;?></p>
	<hr class="my-4">
	<p><?php echo _PROMOTION;?></p>
	<p class="lead">
	<a class="btn-large waves-effect waves-light orange animated flipInX" href=".?r=site/rechercher"><?php echo _MORE_INFO;?></a>
	</p>
</div> -->

<div class="section no-pad-bot">
	<div class="container">
		<h1 class="header center orange-text"><?php echo _ACCUEIL;?></h1>
		<div class="row center">
			<h4 class="header col s12 light"><?php echo _BIENVENUE;?></h4>
		</div>
		<hr>
		<div class="row center light flow-text">
			<p><?php echo _PROMOTION;?></p>
		</div>
		<div class="row center">
			<a class="btn-large waves-effect waves-light orange animated flipInX" href=".?r=site/rechercher"><?php echo _MORE_INFO;?></a>
		</div>
	</div>
</div>


<div class="container">
	<div class="section">

		<!--   Icon Section   -->
		<div class="row">
			<div class="col s12 m4">
				<div class="icon-block">
					<h2 class="center light-blue-text"><i class="material-icons">file_upload</i></h2>
					<h5 class="center"><?php echo _IMPORT_TITLE;?></h5>

					<p class="light"><?php echo _IMPORT_TEXT;?></p>
				</div>
			</div>

			<div class="col s12 m4">
				<div class="icon-block">
					<h2 class="center light-blue-text"><i class="material-icons">add_to_photos</i></h2>
					<h5 class="center"><?php echo _ADD_TITLE;?></h5>

					<p class="light"><?php echo _ADD_TEXT;?></p>
				</div>
			</div>

			<div class="col s12 m4">
				<div class="icon-block">
					<h2 class="center light-blue-text"><i class="material-icons">flash_on</i></h2>
					<h5 class="center"><?php echo _SEARCH_TITLE;?></h5>

					<p class="light"><?php echo _SEARCH_TEXT;?></p>
				</div>
			</div>
		</div>

	</div>
	<br>
</div>
